...
```
.claude/skills/job-posting/
├── SKILL.md                          # 進め方 (情報収集 → 執筆 → 検品 → 出力)
├── assets/job-posting-template.md    # ヒアリング項目と原稿の型
└── references/
...
入稿前に必ず `references/legal.md` のチェック表を全項目当てる。落としやすいのは
固定残業代の 3 点セット (額・相当時間数・超過分別途支給)、就業場所と業務の
**変更の範囲** (2024/4/1 施行の明示ルール)、有期契約の更新上限と無期転換の 3 つ。
