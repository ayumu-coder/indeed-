...
└── skills/job-posting/
    ...
    ├── assets/job-posting-template.md
    ├── references/
    │   ├── legal.md                  # 法令チェック 12 項目、年齢制限の例外
    │   ├── writing.md                # 文字数・職種名・検索対策・記入例
    │   └── bulk.md                   # 大量作成の分解・委任・横断チェック
    └── scripts/lint_posting.py       # 機械チェック (依存なし、Python 3.11+)
```
...
3 件以上の依頼では、指示役 (Opus) が入力の分解・委任・横断チェック・法令判断の
最終ゲートを持ち、実務役 (Sonnet) が 1 件ずつ原稿を書く。1 セッションで順に書くと
後半の原稿が前半に引きずられて似た文面になり、媒体の重複判定に触れるため、
実務役は求人 1 件につき 1 つ並列に起動する。
...
```bash
python3 .claude/skills/job-posting/scripts/lint_posting.py out/ --min-wage 1114
```

必須項目の欠落、固定残業代の 3 点セット、就業場所と業務の**変更の範囲**
(2024/4/1 施行の明示ルール)、有期契約の更新上限と無期転換、年齢・性別・国籍による
限定表現、最低賃金割れ、未解決プレースホルダ、原稿の重複を検出する。
エラーがあると終了コード 1 を返す。標準ライブラリのみで動き、`npm test` とは独立。

リンタが見るのは「書いてあるか」だけなので、文章の質と年齢制限の例外該当性は
指示役が最終判断する。
