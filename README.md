...
`.claude/skills/job-posting/` は Indeed 掲載用の求人原稿を作る Claude Code スキル。
コードからは参照しておらず、`npm test` の対象外。

```
.claude/
├── settings.json                     # 指示役のモデルを opus に固定
├── agents/job-posting-writer.md      # 実務役 (sonnet)。1 件の原稿執筆を担当
└── skills/job-posting/
    ├── SKILL.md                      # 進め方 (件数で分岐 → 執筆 → 検品 → 出力)
    ├── assets/
    │   ├── indeed-upload-columns.md  # 67 列の仕様と選択肢
    │   └── job-posting-template.md   # ヒアリング項目
    ├── references/
    │   ├── house-style.md            # 実データ 359 件から抽出した文体の型
    │   ├── legal.md                  # 法令チェック 12 項目、年齢制限の例外
    │   ├── writing.md                # 文字数・職種名・検索対策
    │   └── bulk.md                   # エリア展開型の分解・委任・横断チェック
    └── scripts/
        ├── rules.py                  # NG 表現の定義 (2 つのリンタで共有)
        ├── lint_sheet.py             # アップロード用シートの検査 (本番)
        └── lint_posting.py           # md 下書きの検査
```

納品物は **Indeed 一括アップロード用シートの行 (67 列)**。読みやすい md ではない。
列の仕様・選択肢と文体の型は、実際に掲載している 359 件から起こしてある。

「求人原稿を作って」「この募集要項を見て」と頼めば自動で起動する。
明示的に呼ぶなら `/job-posting`。新規作成と既存原稿の添削の両方に対応する。

## 役割分担

指示役 (Opus) が入力の分解・委任・横断チェック・法令判断の最終ゲートを持ち、
実務役 (Sonnet) が本文を書く。エリア展開型 (1 企業 1 職種 × N 拠点) では
求人ごとに変わるのは職種名の末尾・勤務地・アクセスの 3 列だけなので、
実務役には**本文パターンを 2〜3 本**作らせ、全行への展開は指示役がスクリプトで行う。
本文が 1 パターンだと全件が完全同一になり、媒体の重複判定に触れる。

指示役のモデルは `.claude/settings.json` の `model` でリポジトリ全体に固定している
(このリポジトリでの作業すべてに効く)。変更は `/config` から。
...
xlsx / csv / tsv を読み、行ごとに次を検出する。エラーがあると終了コード 1 を返す。

- 必須列の空欄、選択肢にない値 (取り込みで弾かれる)
- 固定残業代ありなのに額・時間数・超過分同意が空
- 就業場所と業務の**変更の範囲**の未記載 (2024/4/1 施行の明示ルール)
- 年齢・性別・国籍・身体的要件による限定表現、例外事由の併記有無
- 最低賃金割れ (月給・日給は時給換算して比較)
- タグ「残業なし」と本文の残業時間記載の矛盾
- 職種名の地名が勤務地・アクセスに現れない (駅名と住所の行ズレ検出)
- 仕事内容が完全同一の求人がしきい値を超えている

標準ライブラリのみで動き、`npm test` とは独立。文章の質と年齢制限の例外該当性は
機械では判断できないため、指示役が最終判断する。
